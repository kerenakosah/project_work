...
class FirebaseCaller {
  ...
  Future<String> addDataToCollection({
    ...
    final DocumentReference documentReference = await reference.add(data);
  ...
  Future<T> getCollectionData<T>({
    required String path,
    required Future<T> Function(List<QueryDocumentSnapshot<Map<String, dynamic>>>? data) builder,
  ...
  Future deleteAllCollectionData<T>({
    ...
    Query<Map<String, dynamic>> query = _firebaseFirestore.collection(path).limit(batchSize);
  ...
  // stop document stream
  void stopDocumentStream<T>({
    required String path,
  }) {
    final DocumentReference<Map<String, dynamic>> reference = _firebaseFirestore.doc(path);
